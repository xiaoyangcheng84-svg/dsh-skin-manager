...
import { readFileSync, readdirSync, existsSync, writeFileSync, renameSync, mkdirSync, symlinkSync } from 'node:fs'
...
/** Packages that are not themes even though their name/description may mention skin/theme. */
const NON_THEME_PACKAGES = new Set([
  'dsh-skin-manager',
  'dsh-skin-center',
  'dshmarket',
  'dsh-better-sidebar',
...
/**
 * Read a theme plugin that does not ship skin.json (e.g. dsh-kimino-theme,
 * dsh-skin / Codex-style skin). It must be a web client bundle whose
 * cordis.patch.yml inserts a loader row, and its package name, loader row id,
 * or (when immediately-loaded) description/keywords should look like a
 * theme/skin. Known non-theme utilities are excluded via NON_THEME_PACKAGES.
 */
function readThemeMetaAt(absDir, fallbackPackageName) {
  try {
    const manifest = JSON.parse(readFileSync(join(absDir, 'package.json'), 'utf8'))
    const pkgName = typeof manifest.name === 'string' ? manifest.name : fallbackPackageName
    if (!pkgName || NON_THEME_PACKAGES.has(pkgName)) return null
    if (!manifest.dsh?.client || typeof manifest.dsh?.bundle?.patch !== 'string') return null
    const patchRel = manifest.dsh.bundle.patch
    const patch = readFileSync(join(absDir, patchRel), 'utf8').replace(/^\uFEFF/, '')
    const idm = /-\s*insert:\s*\n\s*-\s*id:\s*['"]?([A-Za-z0-9_-]+)['"]?/.exec(patch)
    if (!idm) return null
    const rowId = idm[1]
    // Recognize a no-skin.json theme when the package/row id mentions
    // theme/skin, or when the description/keywords do AND the client is
    // marked immediately-loaded (common for market themes). Known non-theme
    // utilities are excluded above.
    const text = [manifest.description || '', Array.isArray(manifest.keywords) ? manifest.keywords.join(' ') : ''].join(' ')
    const immediately = manifest.dsh.client.immediately === true
    if (!/theme|skin/i.test(pkgName) && !/theme|skin/i.test(rowId) && !(/theme|skin/i.test(text) && immediately)) return null
    const id = rowId.startsWith('ui-skin-')
      ? rowId.slice('ui-skin-'.length)
      : rowId
    const shortName = manifest.description
      ? manifest.description.split(/[:：—-]/)[0].trim()
      : pkgName
    return {
      id,
      name: shortName || pkgName,
      nameEn: pkgName,
      ...
      order: 1000,
      preview: null,
      wiring: { id: rowId, bundleWired: false },
      dir: absDir,
      themeFallback: true,
    }
  } catch {
...
/** Absolute path of dsh-market's persisted skin state. */
function marketStateFile(dir) {
  return join(dir, '.dsh-market', 'state.json')
}

/** Read dsh-market's persisted disabled-skin package names, or an empty set. */
function readMarketDisabled(dir) {
  try {
    const data = JSON.parse(readFileSync(marketStateFile(dir), 'utf8'))
    return new Set(Array.isArray(data.disabledSkins) ? data.disabledSkins : [])
  } catch {
    return new Set()
  }
}

/** Write dsh-market's persisted disabled-skin package names. */
function writeMarketDisabled(dir, disabled) {
  mkdirSync(join(dir, '.dsh-market'), { recursive: true })
  writeFileSync(marketStateFile(dir), JSON.stringify({ disabledSkins: [...disabled] }))
}

let marketHotModule = null

/**
 * Load dsh-market's hot-mount helpers when the market is installed. The
 * resolved URL is the same file dsh-market itself imports, so the module
 * instance (and its live hot-mount registry) is shared with the market.
 */
async function loadMarketHot(dir) {
  if (marketHotModule !== null) return marketHotModule
  const hotPath = join(dir, 'node_modules', 'dshmarket', 'lib', 'hot.js')
  if (!existsSync(hotPath)) {
    marketHotModule = null
    return null
  }
  try {
    marketHotModule = await import(pathToFileURL(hotPath).href)
  } catch (error) {
    console.error('[ui-skin-manager] dsh-market hot.js unavailable:', error)
    marketHotModule = null
  }
  return marketHotModule
}

/**
 * Synchronize dsh-market's persisted (and when available, live hot-mount)
 * state with the skin-manager's choice. This prevents the market from
 * re-disabling the manager-applied skin at boot and stops stale hot-mounted
 * skins from overlapping the newly applied one.
 */
async function syncMarketState(dir, skins, activeSkin) {
  const hot = await loadMarketHot(dir)
  const disabled = hot ? hot.readDisabledThemes(dir) : readMarketDisabled(dir)
  const activePkg = activeSkin?.package ?? null

  for (const skin of skins) {
    if (activePkg !== null && skin.package === activePkg) disabled.delete(skin.package)
    else disabled.add(skin.package)
  }

  if (hot) hot.writeDisabledThemes(dir, disabled)
  else writeMarketDisabled(dir, disabled)

  if (hot) {
    const mounts = new Set(hot.listHotMounts())
    for (const skin of skins) {
      if (!mounts.has(skin.package)) continue
      if (activePkg !== null && skin.package === activePkg) continue
      try {
        await hot.hotUnmount(skin.package)
      } catch (error) {
        console.error(`[ui-skin-manager] failed to unmount ${skin.package}:`, error)
      }
    }
  }
}
...
export function listSkins(dir = profileDir()) {
  ...
  const addSkin = (absDir, packageName) => {
    ...
    let meta = readSkinMetaAt(absDir, packageName)
    if (meta === null && !existsSync(join(absDir, SKIN_JSON))) {
      meta = readThemeMetaAt(absDir, packageName)
    }
    if (meta === null) return
 ...
 */
/** Whether a skin's own bundle patch actually inserts its loader row. */
function bundlePatchInsertsRow(skin, dir) {
  try {
    const manifest = JSON.parse(readFileSync(join(dir, 'node_modules', skin.package, 'package.json'), 'utf8'))
    const patchRel = manifest.dsh?.bundle?.patch
    if (typeof patchRel !== 'string') return false
    let content = readFileSync(join(dir, 'node_modules', skin.package, patchRel), 'utf8')
    if (content.charCodeAt(0) === 0xFEFF) content = content.slice(1)
    const rowId = rowIdFor(skin)
    const re = /-\s*insert:\s*\n((?:\s+.*\n?)*)/g
    let m
    while ((m = re.exec(content)) !== null) {
      if (new RegExp(`^\\s*-\\s*id:\\s*['"]?${rowId}['"]?\\s*$`, 'm').test(m[1])) return true
    }
  } catch { /* fall through */ }
  return false
}
...
function isBundleWired(skin, dir) {
  ...
  return profileBundles(dir).has(skin.package) && bundlePatchInsertsRow(skin, dir)
}

/** Which skin/theme is currently active (enabled patch row, bundle wire, or live market hot-mount). */
export async function currentActive(dir = profileDir()) {
  const skins = listSkins(dir)

  // A dsh-market hot mount is the live truth for a just-installed theme, even
  // before the profile patch has caught up.
  const hot = await loadMarketHot(dir)
  if (hot) {
    const mounts = new Set(hot.listHotMounts())
    const hotSkin = skins.find((s) => mounts.has(s.package))
    if (hotSkin) return hotSkin.id
  }
  ...
  if (content.charCodeAt(0) === 0xFEFF) content = content.slice(1)
  // Collect disabled and inserted loader rows generically; only rows belonging
  // to discovered skins/themes are considered below.
  const disabled = new Set()
  for (const m of content.matchAll(/^- id: ([^\s#]+)\n\s*disabled: true/gm)) {
    disabled.add(m[1])
  }
  const inserted = []
  for (const m of content.matchAll(/-\s+id:\s*['"]?([^\s#'"]+)/g)) {
    if (!disabled.has(m[1])) inserted.push(m[1])
  }
  const insertedSkins = skins.filter((s) => inserted.includes(rowIdFor(s)))
  if (insertedSkins.length > 0) return insertedSkins[insertedSkins.length - 1].id
  // No skin-manager insert row: a bundle-wired skin/theme may already be
  // active via its own bundle patch.
...
export async function applySkin(skin, dir = profileDir()) {
  const file = patchFile(dir)
  const skins = listSkins(dir)
  ...
  // Keep dsh-market's persisted/live state in sync so it cannot re-disable
  // the skin we just activated (or keep a stale hot mount alive).
  await syncMarketState(dir, skins, active)
  return active
}
...
export function makeRoutes() {
  ...
  /**
   * Ask dsh-market to update its in-memory disabled set BEFORE the profile
   * patch is rewritten. If we only update state.json after the patch, the
   * config watcher can enable the target while the market still thinks it is
   * disabled; the market's self-healing guard then disables it again a moment
   * later. The sync-skin route is state-only and works even when the target's
   * loader entry does not exist yet (e.g. deep-whale / maid-atelier, whose
   * bundle patches only augment a manager-inserted row).
   */
  const syncMarketBeforeApply = async (req, skins, target) => {
    try {
      const host = req.headers.host
      if (!host) return
      const activePkg = target?.package ?? null
      const disabled = skins
        .filter((s) => activePkg === null || s.package !== activePkg)
        .map((s) => s.package)
      const controller = new AbortController()
      const timer = setTimeout(() => controller.abort(), 5000)
      try {
        await fetch(`http://${host}/dsh-market/sync-skin`, {
          method: 'POST',
          headers: { 'content-type': 'application/json', origin: `http://${host}` },
          body: JSON.stringify({ active: activePkg, disabled }),
          signal: controller.signal,
        })
      } finally {
        clearTimeout(timer)
      }
    } catch { /* market absent or old version — applySkin's state-file sync still covers boot */ }
  }
  ...
  return [
    {
      ...
      handler: async (req, res) => {
        ...
        const skins = listSkins(dir)
        const active = await currentActive(dir)
      ...
      handler: (req, res) => {
        ...
        req.on('end', async () => {
          try {
            ...
            // Tell dsh-market first: if the market still thinks the target is
            // disabled, the config watcher can enable the patch row and the
            // market's self-healing guard would immediately disable it again.
            // Syncing the market's in-memory state before rewriting the patch
            // avoids that race.
            await syncMarketBeforeApply(req, skins, target)
            const applied = await applySkin(target, dir)
