...
/** Weak theme-name heuristic for plugins without skin.json. */
const THEME_HINT_RE = /theme|skin/i

/**
 * Read a theme plugin that does not ship skin.json (e.g. dsh-kimino-theme).
 * It must be a web client bundle whose cordis.patch.yml inserts a loader row,
 * and its package name/description/keywords should look like a theme/skin.
 */
function readThemeMetaAt(absDir, fallbackPackageName) {
  try {
    ...
    const haystack = [pkgName, manifest.description || '', Array.isArray(manifest.keywords) ? manifest.keywords.join(' ') : ''].join(' ')
    if (!THEME_HINT_RE.test(haystack)) return null
    ...
    return {
      id,
      name: manifest.description ? manifest.description.split(/[—-]/)[0].trim() : pkgName,
      nameEn: pkgName,
...
/** Which skin/theme is currently active (the enabled insert row in the patch). */
