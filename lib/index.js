 ...
 * and its package name or loader row id should contain "theme" — this avoids
 * picking up utilities like dsh-skin that merely mention skin/theme in text.
 */
function readThemeMetaAt(absDir, fallbackPackageName) {
  try {
    ...
    if (!idm) return null
    const rowId = idm[1]
    // Require the theme marker in the package name or the loader row id.
    if (!/theme/i.test(pkgName) && !/theme/i.test(rowId)) return null
